...
class Close:
    try:
        ...
        def on_closing(self):
            if messagebox.askokcancel("Lời Tạm Biệt", "Hôm nay, bạn làm tốt lắm! \n Cố gắng hơn nhá !"):
                with open("datastorage/score.txt", "a", encoding="utf-8") as file:
                    file.write(
                        f"{(self.score / self.progress['maximum']) * 10},{self.score},{self.progress['maximum']}\n")
        ...
        def exit(self):
            with open("datastorage/score.txt", "a", encoding="utf-8") as file:
                file.write(f"{(self.score / self.progress['maximum']) * 10},{self.score},{self.progress['maximum']}\n")
            with open("datastorage/env.txt", "r", encoding="utf-8") as file:
                date = file.readlines()
            day = date[0].split()
            if (int(datetime.datetime.now().weekday()) != int(day[1].strip()) and int(
...
class SpacedRepetitionScheduler:
    try:
        ...
        def add_item(self, name, difficulty):
            self.items.append({"name": name, "difficulty": difficulty})
...
class LearnApp:
    try:
        def __init__(self, root):
            ...
            if int(datetime.datetime.now().weekday()) == int(day[0]) or int(datetime.datetime.now().weekday()) == int(
                ...
                try:
                    ...
                    print(e)
                self.title = "Ôn tập lại các từ bạn biết gần đây"
                print("learn")
            elif int(datetime.datetime.now().weekday()) == int(day[2]) or int(datetime.datetime.now().weekday()) == int(
                    day[3]):  # loadlist
                if self.check_log(datetime.datetime.now().strftime("%Y-%m-%d")):
                    ...
                    self.checkok()
                    self.vocabulary = list(set(self.reviseload()))
                self.title = "Học các nhóm từ vựng mới"
                print("load list")
            elif int(datetime.datetime.now().weekday()) == int(day[4]) or int(datetime.datetime.now().weekday()) == int(
                    day[5]) or int(datetime.datetime.now().weekday()) == int(day[6]):  # reviselist
                self.clearcheck()
                self.vocabulary = list(set(self.reviseload()))
                self.title = "Ôn lại các nhóm từ mới gần đây"
                print("revise list")
            if int(datetime.datetime.now().weekday()) == 0 and not self.checkran(
                ...
                self.tick()
            self.root.protocol("WM_DELETE_WINDOW", self.on_closing)
            self.root.title("Learn Vocabulary")
            self.root.geometry("1000x700+300+50")
            ...
            self.textbox = tk.Text(root, width=50, height=13, wrap=tk.WORD, font=("Helvetica", 34))
        ...
        def loadtext(self):
            ...
            self.textbox.insert(tk.END, vocab_list)
        ...
        def load_learn(self):
            ...
            if len(lines) == 0:
                ...
                percentage = 0.2
                num_lines = len(li)
                num_lines_to_extract = int(num_lines * percentage)
                lines = li[-num_lines_to_extract:]
        ...
        def load_list(self):
            listname = []
            pos = []
            with open("datastorage/revise.txt", "w", encoding="utf-8") as file:
                pass
            with open("datastorage/delrevise.txt", "w", encoding="utf-8") as nfile:
                pass
            ...
            for idx in range(len(listname)):
                ...
                if self.checktype(learnline):
                    ...
                    fr = st + int(pos[idx])
                    # print(f"{fr}")
                    stop = fr + random.randint(10, 18)
                    # print(f"{stop}")
                    vocabline = lines[fr:stop]
...
if __name__ == "__main__":
    '''
        ...
        app = LearnApp(root)
        root.mainloop()
    elif int(datetime.datetime.now().weekday()) == 0:
        root = tk.Tk()
        app = RememberApp(root)
        root.mainloop()
    elif int(datetime.datetime.now().weekday()) == 2:
        root = tk.Tk()
        app = RememberApp(root)
        root.mainloop()
    elif int(datetime.datetime.now().weekday()) == 5:
        root = tk.Tk()
        app = RememberApp(root)
        root.mainloop()'''
